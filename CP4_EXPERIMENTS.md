...
**Final result: HitRate@10 `1.000`, MRR `0.856365`, MTTC `1.885`,
TechnicalScore `0.939210`** — `+0.006590` over the CP3 baseline, with the only
CP3 miss (`public_0020`) fixed and no session regressed below it.

| Deliverable | Location |
|---|---|
| Experiment table | this file |
| Retrained weights (18 features, C=0.1) | `starter/reranker_weights.json` |
| Cross-validation report (17 configs, per-feature outcomes) | `docs/cp4_cv_report.json` |
| Paraphrase report (clean vs paraphrased) | `docs/cp4_paraphrase_report.json` |
| Test suite | `python3 -m unittest discover tests` — 38 tests, green |
| Evaluator | untouched (`git diff 2396dce HEAD -- evaluator/ data/public_set.jsonl` is empty) |
